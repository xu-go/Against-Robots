...
uint8_t CAN1_mode_init(uint8_t tsjw, uint8_t tbs2, uint8_t tbs1, uint16_t brp, uint8_t mode)
{
    ...
    CAN_FilterInitTypeDef CAN_FilterInitStructure;//筛选器初始化结构体
...
//CAN波特率设置
    CAN_InitStructure.CAN_SJW = tsjw;  //重新同步跳跃时间单元
    CAN_InitStructure.CAN_BS1 = tbs1; //时间段1的单元时间
    CAN_InitStructure.CAN_BS2 = tbs2; //时间段2的单元时间
    CAN_InitStructure.CAN_Prescaler = brp;//波特率分频   
