...
{
  S1=7.4*190+500; //变大，往下220  （上限120）  190          140     增益环100
	S2=7.4*40+500; //变大，往前50（上限210）      40     三倍区150            160 
	S3=7.4*130+500;//变小，夹紧                   200                         150
}
//200
void Nor(void) //初始化状态
{
...
void Normal2(void) //正常行驶状态（带环）
{
	if(flag.arm == TRUE)
	{
		...
		Servo_Target[2] = 120;   //GET_RING;150
		Servo_Ramp(servo1,10);
		Servo_Ramp(servo2,10);
		Servo_Ramp(servo3,10);
		servo(Servo_Real[0],Servo_Real[1],Servo_Real[2]);
...
void Get(void) //抓到环
{
	if(flag.arm == TRUE)
	{
		Servo_Target[0] = 40;//110
		Servo_Target[1] = 180;
		Servo_Target[2] = 120;    //GET_RING;140
		Servo_Ramp(servo1,10);
		Servo_Ramp(servo2,10);
		Servo_Ramp(servo3,10);
		TIM5_Servo(Servo_Real[0],Servo_Real[1],Servo_Real[2]);
...
void Place222(void) //放环（三倍区）对抗侧放
{
	if(flag.arm == TRUE)
	{
		Servo_Target[0] = 120;//150
		Servo_Target[1] = 135;//150
...
void Place11(void) //放环（二倍区）的第一步
{
	if(flag.arm == TRUE)
	{
		Servo_Target[0] = 130;//110
		Servo_Target[1] = 160;//150
		Servo_Target[2] = 130;
		Servo_Ramp(servo1,10);
		Servo_Ramp(servo2,10);
		Servo_Ramp(servo3,10);
